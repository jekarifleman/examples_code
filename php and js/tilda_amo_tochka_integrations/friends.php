    ...
    if (authorize($settings, $settings['subdomain'])) {
        ...
        if ($contactId === '') {
            ...
            if ($contactId !== '') {
                ...
                // статус "первичный контакт" в воронке "рекомендации", на котором будет создана сделка
                $statusId = $settings['status_rekomendacii_pervichniy_kontakt'];
                ...
                if (($leadId !== false) && ($leadId !== '')) {
                    file_put_contents(__DIR__ . "/logs/logs.txt", date("Y-m-d h:i:s ", time()) . " Создана сделка {$leadId} в разделе thanks\n", FILE_APPEND);
                } else {
                    file_put_contents(__DIR__ . "/logs/logs.txt", date("Y-m-d h:i:s ", time()) . " Ошибка при создании сделки в разделе thanks\n", FILE_APPEND);
                ...
                file_put_contents("{$basePath}/logs/errors.txt", date("Y-m-d H:i:s ", time()) . "\tПроизошла ошибка при создании контакта для раздела thanks\n", FILE_APPEND);
                echo json_encode(['contact_id' => "error"]);
                exit(0);
            }
            ...
            // массив с контактом будет отправлен в ответ на запрос после создания сделки
            $contact = ['contact_id' => $contactId];
        ...
        // отвечаем ajax-запросу об ошибке
        echo json_encode(Array('contact_id' => 'error'));
        exit(0);
    }
} else {
